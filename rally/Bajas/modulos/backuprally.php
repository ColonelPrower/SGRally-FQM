<!-- Sistema de Administracion de Rally de Conocimientos de Fisica, Quimica y Matematicas
    Script para eliminar y respaldar datos a un historial y reparar posibles conflictos en la base de datos
...
<?php
$consulta=mysqli_query($conn,"Select * from ranking") or die("no se pudo respaldar el ranking".mysqli_error($conn));
while($row=mysqli_fetch_array($consulta)){
	$num_eq=$row['num_equipo'];
	$duracion=$row['duracion'];
	$aciertos=$row['aciertos'];
	$fecha=$row['fecha'];
	$consulta2=mysqli_query($conn,"Select * from equipos where num_equipo='$num_eq'") or die("error de nombre".mysqli_error($conn));
	$row2=mysqli_fetch_array($consulta2);
	$nombre=$row2['nombre'];
	$representante=$row2['representante'];
	mysqli_query($conn,"Insert into historial values('$num_eq', '$nombre','$representante' , '$duracion', '$aciertos', '$fecha')") or die("no se pudo hacer backup".mysql_error());
}
