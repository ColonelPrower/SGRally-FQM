...
function FancyTable($encabezados,$result)
{
    ...
    do {
	   ...
	   // Cell(puntos, puntos, string, 0no y 1si, 0der 1siglinea 2abajo, 'C''R''L', boolean ) 
		...
		$this->Cell($w[5],15,$row[5],0,0,'C',$fill);
...
$result = mysqli_query($conn,"SELECT num_equipo, nombre, nivel, representante, fecha_reg, calendario from equipos"); //devuelve true si se pudo hacer la consulta, y falso en caso contrario
