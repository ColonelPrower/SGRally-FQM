<!-- Sistema de Administracion de Rally de Conocimientos de Fisica, Quimica y Matematicas
   Script de modificacion de Profesores
...
<html>
	...
	<body>
		...
		$rutafoto="../../images/maestros/$nombredefoto";
		if(isset($_FILES['foto']['name'])){
			$fotoprof=$_FILES['foto']['name'];
			if($_FILES['foto']['type']!=="image/jpeg" and $_FILES['foto']['name']!==''){
				die("La foto del maestro debe ser jpg o jpeg");
			}
		}
		 else{
			$fotoprof='';
		...
		mysqli_query($conn,"Update maestros set  nombre='$nombre' , telefono='$telefono' , foto='$nombrefoto' , materia='$materia'  where codigo='$codigo'") or die("no se pudo modificar el maestro ".mysqli_error($conn));
		echo("maestro modificado");
		?>
...
<?php }
	...
	echo ("Est&aacutes accediendo a una p&aacutegina restringida, para ver su contenido debes estar registrado.<br /><a href='../../login/acceso.php'>Ingresar</a>");
	}
?>